...
# Initialize list to store importance_rsf
importance_rsf_list <- list()
...
for (alpha in best_alpha_values) {
  ...
  for (i in seeds) {
    ...
    RSF_fit <- fit_RSF(genes_expression_train, survival_object_train, alpha, survival_train, thresholds_list)
    
    # Save importance_rsf for this seed
    alpha_importance_list[[as.character(i)]] <- RSF_fit$importance
  ...
  # Store importance_rsf values for this alpha
  importance_rsf_list[[as.character(alpha)]] <- alpha_importance_list
}
...
ggsave(filename = "RSF_Optimization_Results/plot_thresholds.pdf", plot = plot_all, width = 6, height = 4)
# write on the chunk end time file


# Plot a the density distribution of the VIMP for each alpha value

# Combine all importance_rsf values into a single data frame
importance_combined <- data.frame()

for (alpha in names(importance_rsf_list)) {
  flat_values <- unlist(importance_rsf_list[[alpha]])
  temp_df <- data.frame(
    importance = flat_values,
    Alpha = alpha
  )
  importance_combined <- rbind(importance_combined, temp_df)
}

# Optional: Remove zero importances (commonly added by RSF for non-selected features)
#importance_combined <- importance_combined %>% filter(importance > 0.004)

# Convert Alpha to a factor for ordered plotting (optional)
importance_combined$Alpha <- factor(importance_combined$Alpha)

plot_vimp <- ggplot(importance_combined, aes(x = importance, fill = Alpha, color = Alpha)) +
  geom_density(alpha = 0.2, size = 1.2, position = "identity") +
  scale_fill_brewer(palette = "Set1") +
  scale_color_brewer(palette = "Set1") +
  theme_minimal() +
  labs(
    title = "Distribution of Feature VIMP Across 10 Seeds",
    x = "VIMP Score",
    y = "Density"
  )

ggsave(filename = "RSF_Optimization_Results/plot_vimp_density.pdf", plot = plot_vimp, width = 6, height = 4)

plot_vimp_faceted <- ggplot(importance_combined, aes(x = importance, fill = Alpha)) +
...
ggsave(filename = "RSF_Optimization_Results/plot_vimp_faceted.pdf", plot = plot_vimp_faceted, width = 8, height = 4)
