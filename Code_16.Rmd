...
for (i in seq_along(seeds)) {
  logger::log_info("Running analysis for seed: {seeds[i]}")
  ...
  for (j in seq_along(models_coefficients_list[[i]])) {
    ...
    if (num_coeff_df[j, i] > 0) {
      ...
      logger::log_info("seed: ", seeds[i])
      logger::log_info("c_index: ", c_index_df[j, i])
      logger::log_info("IBS: ", IBS_df[j, i])
      num_coeff_df[j, i] <- length(models_coefficients_list[[i]][[j]]$ensembl_gene_id)
    } else {
...
best_model_coefficients <- models_coefficients_list[[2]][[best_model_index]] # o index aqui escolhido tem que bater certo com o seed do split data para que dê igual performance
...
for (i in seq_along(seeds_all_equal)) {
  ...
  for (j in seq_along(models_coefficients_list_all_equal[[i]])) {
    ...
    if (num_coeff_df_all_equal[j, i] > 0) {
      ...
      logger::log_info("c_index: ", c_index_df_all_equal[j, i])
      logger::log_info("IBS: ", IBS_df_all_equal[j, i])
      num_coeff_df_all_equal[j, i] <- length(models_coefficients_list_all_equal[[i]][[j]]$ensembl_gene_id)
