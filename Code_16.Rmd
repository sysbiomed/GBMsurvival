...
# Local chunk to store shared dataset
feather::write_feather(genes_expression, "Results/genes_expression.feather")
feather::write_feather(survival_data, "Results/survival_data.feather")
```
...
best_model_index <- 3#which.max(rowMeans(c_index_df, na.rm = TRUE)) 
...
best_model_index_all_equal <- 3#which.min(rowMeans(p_value_df_all_equal, na.rm = TRUE)) 
...
if (nrow(as.data.frame(reactome_results)) > 0) {
  dotplot(reactome_results, showCategory = 10)  # Show top 10 GO terms
} else {
...
write.csv(mapped_and_unmapped_genes, file = "Results/mapped_and_unmapped_genes.csv", row.names = FALSE)

# Map to STRING IDs (remove unmapped rows)
mapped_genes <- string_db$map(genes_symbols, "SYMBOL", removeUnmappedRows = TRUE)
write.csv(mapped_genes, file = "Results/mapped_genes.csv", row.names = FALSE)
...
if (nrow(mapped_genes) > 0) {
  string_db$get_png(mapped_genes$STRING_id,
                    file = "Results/ppi_network.png")  # Or "ppi_network.pdf"
