...
```{r Fit_and_Explore_the_Models_different, warning=FALSE}
#message=FALSE, 
# Models' names
models_names_list <- c("Cox", "RandomForest+Cox", "CausalForest+Cox", "Boruta+Cox") #"SIS", "ISIS", 
...
colnames(c_index_df) <- paste("Seed_", seeds)
rownames(c_index_df) <- models_names_list

# Create an empty dataframe to store p-value
p_value_df <- data.frame(matrix(ncol = length(seeds), nrow = num_models))
colnames(p_value_df ) <- paste("Seed_", seeds)
rownames(p_value_df ) <- models_names_list

# Create an empty dataframe to store Integrated Brier Score (IBS)
IBS_df <- data.frame(matrix(ncol = length(seeds), nrow = num_models))
colnames(IBS_df) <- paste("Seed_", seeds)
...
colnames(num_coeff_df) <- paste("Seed_", seeds)
...
best_model_index <- which.min(rowMeans(p_value_df, na.rm = TRUE)) #4
#best_model_index <- which.max(rowMeans(c_index_df, na.rm = TRUE)) #4
best_model_name <- rownames(p_value_df)[best_model_index]

logger::log_info("Best model:", best_model_name)

# Get the coefficients of the best model
best_model_coefficients <- models_coefficients_list[[5]][[best_model_index]] # o index aqui escolhido tem que bater certo com o seed do split data para que dê igual performance
...
# Split the data
splited <- splitTestAndTrain(genes_expression, survival_data, 0.7, 5) # <- colocar o seed
...
colnames(c_index_df_all_equal) <- paste("Seed_", seeds_all_equal)
rownames(c_index_df_all_equal) <- models_names_list

# Create an empty dataframe to store C-index values
p_value_df_all_equal <- data.frame(matrix(ncol = length(seeds_all_equal), nrow = num_models_all_equal))
colnames(p_value_df_all_equal) <- paste("Seed_", seeds_all_equal)
rownames(p_value_df_all_equal) <- models_names_list

# Create an empty dataframe to store Integrated Brier Score (IBS)
IBS_df_all_equal <- data.frame(matrix(ncol = length(seeds_all_equal), nrow = num_models_all_equal))
colnames(IBS_df) <- paste("Seed_", seeds_all_equal)
rownames(IBS_df) <- models_names_list

# Create an empty dataframe to store the number of coefficients
num_coeff_df_all_equal <- data.frame(matrix(ncol = length(seeds_all_equal), nrow = num_models_all_equal))
colnames(num_coeff_df_all_equal) <- paste("Seed_", seeds_all_equal)
...
for (i in seeds) {
  ...
  for (j in seq_along(models_coefficients_list_all_equal[[i]])) {
    ...
    if (num_coeff_df_all_equal[j, a] > 0) {
      ...
      c_index_p_value_all_equal <- calculate_c_index(survival_object_train, genes_expression_train, models_coefficients_list_all_equal[[i]][[j]], genes_expression_test, survival_test, survival_data)
...
if (nrow(as.data.frame(go_enrichment)) > 0) {
  ...
  logger::log_info("No enriched GO terms to plot.")
}
...
if (nrow(as.data.frame(kegg_enrichment)) > 0) {
  ...
  logger::log_info("No enriched KEGG terms to plot.")
}
