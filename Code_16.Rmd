...
# Implement the separate2GroupsCox function

risk <- separate2GroupsCox(xdata = as.matrix(subset(genes_expression_test, select = best_model_coefficients$ensembl_gene_id)),
                           ydata = data.frame(time = survival_test$days, status = survival_test$vital_status),
                           chosen.btas = setNames(as.numeric(best_model_coefficients$Coef_value), best_model_coefficients$ensembl_gene_id))
risk$plot
...
riskAUC = risksetAUC(Stime=survival_test$days,
                     ...
                     marker = survival_probabilities_test,
                     method = "Cox",
                     tmax = ceiling(max(survival_test$days)),
                     type = "b",
                     plot = TRUE,
                     xlab = "Time (years)") 
