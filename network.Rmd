...
purrr::walk(string_cutoff[1], \(x) {
  purrr::walk(alpha[1], \(y) {
    rmarkdown::render(
      "network.Rmd", 
      output_file = glue::glue("out/network_{x}_{y}.html"), 
...
box::use(
  ...
  ./Functions/network_utils[string_db_homo_sapiens, string_db_local, my_plot],
)
...
data$ppi <- local({
  ...
  ppi_raw <- glmSparseNet:::run.cache(string_db_local)
  
  ppi_raw_hash <- glmSparseNet:::digest.cache(ppi_raw) # Faster to try different score_thresholds
  ppi_data <- glmSparseNet:::run.cache(glmSparseNet:::calculate.combined.score, ppi_raw, score_threshold = params$string_cutoff, remove.text = TRUE, cache.digest = list(ppi_raw_hash))
...
results <- purrr::map(
  ...
  \(x) {
    ...
    log_rank_result <- survival::survdiff(data$test$ydata ~ risk_group)
...
```{r}
results |> 
  dplyr::mutate(label = sprintf("%s (cindex: %.5f, pvalue: %.5f)", label, c_index, log_rank_pvalue)) |>
