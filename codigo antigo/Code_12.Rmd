...
## [old] Outliers based on residuals

```{r}

remove_outliers <- function(models_coefficients, genes_expression, survival_data) {

# Define survival object
survival_object_all <- Surv(time = survival_data$days, event = survival_data$vital_status)  
    
# Assuming models_coefficients and the fitted models are already defined and available
outliers_list <- list()
    ...
    for (i in seq_along(models_coefficients)) {
      ...
      fit <- coxph(survival_object_all ~ ., 
               data = subset(genes_expression, select = models_coefficients[[i]]$ensembl_gene_id), # specify coefficients 
               init = as.numeric(models_coefficients[[i]]$Coef_value), # specify coefficient values
               iter.max = 0) # force the software to keep those values
      ...
      # Identify outliers based on residuals (using a threshold, e.g., 95th percentile)
      outliers <- which(abs(residuals) > quantile(abs(residuals), 0.95))
      
      # Store the outliers for the current model
      outliers_list[[i]] <- unique(outliers)
}
...
results_df <- find_best_alpha_for_glmnet(alpha_vector, genes_expression[1:100,], survival_data)
...
# Compute time-dependent ROC and AUC with marginal weighting
  time_roc <- timeROC(T = survival_test_clean$days,
                      delta = survival_test_clean$vital_status,
  ...
  # Extract and display AUC values
