...
set.seed(1012)
fit <- cv.glmnet(prepareDataForCoxRegression(genes_expression_train), survival_object, family = "cox", alpha = 1.0)  # alpha = 1 for LASSO, 0 for ridge
...
# Apply the coefficients (predictor) to the train data
survival_probabilities_train <- predict(fit, newx = prepareDataForCoxRegression(genes_expression_train), type = "response", s= fit$lambda.min)

# Find the best threshold
best_threshold <- findBestThreshold(survival_train, survival_probabilities_train)

# Apply the coefficients (predictor) to the test data
survival_probabilities_test <- predict(fit, newx = prepareDataForCoxRegression(genes_expression_test), type = "response", s= fit$lambda.min)

# Categorize individuals of the test data based on the threshold
risk_groups <- ifelse(survival_probabilities_test > best_threshold, "Low", "High")
