...
required_packages <- c(
  ...
  "tidyverse", "tibble", "dplyr", "matrixStats", "grf", "Boruta"
)
...
set.seed(1012)
causal_forest_model <- causal_forest(X, survival_train$days, survival_train$vital_status)
...
## Fit a Cox regression model using the covariates
fit_causal_forest_model <- coxph(survival_object ~ .,
...
# Extract coefficients from the fitted model
CausalForest_Coefficients <- data.frame(Coef_value = as.data.frame(coef(fit_causal_forest_model))[,1],                              ensembl_gene_id = rownames(as.data.frame(coef(fit_causal_forest_model))))
```

# Fit the Boruta Algorithm model

```{r message = FALSE}
# Run Boruta
set.seed(1012) # For reproducibility
boruta_output <- Boruta(survival_object ~ ., data = genes_expression_train, doTrace = 2, maxRuns = 100)

# Get the significant features
significant_features <- getSelectedAttributes(boruta_output, withTentative = TRUE)
...
# Extract coefficients from the fitted model
Boruta_Coefficients <- data.frame(Coef_value = as.data.frame(coef(fit_boruta_model))[,1],                              ensembl_gene_id = rownames(as.data.frame(coef(fit_boruta_model))))
...
```{r}
# Compare the models
models_coefficients <- list(GLMNET_Coefficients, SIS_Coefficients, ISIS_Coefficients, CausalForest_Coefficients, Boruta_Coefficients)
...
model_list <- list(GLMNET_Coefficients, SIS_Coefficients, ISIS_Coefficients, CausalForest_Coefficients, Boruta_Coefficients)
