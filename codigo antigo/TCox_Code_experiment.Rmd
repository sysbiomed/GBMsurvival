...
# Kaplan-Meier for each disease
```{r}
## Kaplan-Meier 
fit_surv_teste <- survfit(Surv(survival_data$days, survival_data$vital_status) ~ survival_data$project_id)
survdiff(Surv(survival_data$days, survival_data$vital_status) ~ survival_data$project_id)
ggsurvplot(fit_surv_teste, data = survival_data)
```
...
```{r}
# selecionar apenas os dados da doença de interesse
data_astro <- selectDataPerDisease("astrocytoma", genes_expression, survival_data)
data_glio <- selectDataPerDisease("glioblastoma", genes_expression, survival_data)                                   
genes_expression_astro <- data_astro$genes_expression
survival_data_astro <- data_astro$survival_data

genes_expression_glio <- data_glio$genes_expression
survival_data_glio <- data_glio$survival_data
...
genes_expression_astro <- cleanGeneExpressionData(genes_expression_astro)
genes_expression_glio <- cleanGeneExpressionData(genes_expression_glio)

# keep only columns commom to both diseases
common_columns <- intersect(names(genes_expression_astro), names(genes_expression_glio))
genes_expression_astro <- genes_expression_astro[, common_columns, drop = FALSE]
genes_expression_glio <- genes_expression_glio[, common_columns, drop = FALSE]

# EdgeR+voom normalization
n_rows_astro <- nrow(genes_expression_astro)
n_rows_glio <- nrow(genes_expression_glio)

genes_expression <- rbind(genes_expression_astro, genes_expression_glio)
genes_expression <- normalizationEdgeR(genes_expression)

genes_expression_astro <- genes_expression[1:n_rows_astro, ]
genes_expression_glio <-genes_expression[(n_rows_astro + 1):(n_rows_astro + n_rows_glio), ]
...
# ordenar os dados por paciente
genes_expression_astro <- orderDataByPatient(genes_expression_astro)
survival_data_astro <- orderDataByPatient(survival_data_astro)

genes_expression_glio <- orderDataByPatient(genes_expression_glio)
survival_data_glio <- orderDataByPatient(survival_data_glio)

# merge the data
genes_expression <- rbind(genes_expression_astro, genes_expression_glio)
survival_data <- rbind(survival_data_astro ,survival_data_glio)
```
# Follow up time analysis for Astro and Glio
```{r}

combined_data <- rbind(
  data.frame(value = survival_data$days, group = "Both"),
  data.frame(value = survival_data_astro$days, group = "Astro"),
  data.frame(value = survival_data_glio$days, group = "Glio")
)

# Create a density plot with different colors for each group
ggplot(combined_data, aes(x = value, fill = group)) +
  geom_density(alpha = 0.3) +
  labs(title = "Density Plot of Days to Last Follow Up",
       x = "Value",
       y = "Density") +
  scale_fill_manual(values = c("Both" = "red", "Astro" = "blue", "Glio" = "green"))
...
# Split the data in train and test
set.seed(1012)
#splited <- splitTestAndTrain(genes_expression_astro, survival_data_astro, 0.7)
splited <- splitTestAndTrain(genes_expression_glio, survival_data_glio, 0.7)
...
# Filter glioblastoma patients who died (non-censored data)
glio_died <- survival_data_glio[survival_data_glio$vital_status == 1, ]

# Determine the median survival time
median_survival <- median(glio_died$days)

# Divide glioblastoma patients into high and low groups based on median survival
glio_high <- glio_died[glio_died$days > median_survival, ]
glio_low <- glio_died[glio_died$days <= median_survival, ]

# Prepare gene expression data for high and low groups
genes_expression_glio_high <- genes_expression_glio[genes_expression_glio$patient %in% glio_high$patient, ]
genes_expression_glio_low <- genes_expression_glio[genes_expression_glio$patient %in% glio_low$patient, ]

# Remove the 'patient' column for correlation calculations
genes_expression_glio_high <- subset(genes_expression_glio_high, select = -patient)
genes_expression_glio_low <- subset(genes_expression_glio_low, select = -patient)

# Calculate correlation matrices for high and low groups
glio_high_cor <- bigcor(genes_expression_glio_high, y = NULL, fun = "cor", size = 2000, verbose=FALSE)
glio_high_cor <- as.data.frame(as.ffdf(glio_high_cor))

glio_low_cor <- bigcor(genes_expression_glio_low, y = NULL, fun = "cor", size = 2000, verbose=FALSE)
glio_low_cor <- as.data.frame(as.ffdf(glio_low_cor))

## angular distance
ang_weight <- vector()

for (i in 1:ncol(genes_expression_glio_high)) {
  ang_weight[i] <- acos(cosine(glio_high_cor[,i], glio_low_cor[,i])) / pi
}

## normalized weights (between 0 and 1)

normalized_weights <- ang_weight / max(ang_weight)
hist(ang_weight)
normalized_weights <- (1 - normalized_weights)^3
hist(normalized_weights, main="(1 - normal weight)^3")
#normalized_weights <- (normalized_weights)^3
#hist(normalized_weights, main="(normal weight)^3")
...
cv.fit_tweleele3 <- twiner_glmnet(prepareDataForCoxRegression(genes_expression_train),
                                  survival_object,0.3, normalized_weights)
##genes
