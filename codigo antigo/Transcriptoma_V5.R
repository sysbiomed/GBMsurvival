...
# fit the Regularized Cox Regression (I)SIS
set.seed(1012)
fit <- SIS(prepareDataForCoxRegression(genes_expression_train),
           survival_object, 
           family = "cox",
...
# Extract coefficients from the fitted model
CoxCoefficients <- data.frame(Coef_value = as.data.frame(fit$coef.est)[,1], 
                              ensembl_gene_id = colnames(prepareDataForCoxRegression(genes_expression_train)[, fit$ix]))
...
# Find the best threshold based on the score (log-rank) test 
...
GBM_ISIS_S <- GBM_ISIS$external_gene_name
GBM_SIS_S <- GBM_SIS$external_gene_name
GBM_NET_S <- GBM_NET$external_gene_name

LGG_ISIS_S <- LGG_ISIS$external_gene_name
LGG_SIS_S <- LGG_SIS$external_gene_name
LGG_NET_S <- LGG_NET$external_gene_name


# Create the Venn diagram
venn.plot <- venn.diagram(
  #x = list(GBM_ISIS_S, GBM_SIS_S, GBM_NET_S),
  x = list(LGG_ISIS_S, LGG_SIS_S, LGG_NET_S),
  category.names = c("ISIS", "SIS", "NET"),
  filename = NULL
)
