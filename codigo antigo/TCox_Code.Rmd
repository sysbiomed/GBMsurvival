...
genes_expression_astro_without_patient <- subset(genes_expression_astro, select = -patient)
genes_expression_glio_without_patient <- subset(genes_expression_glio, select = -patient)

astro_cor <- bigcor(genes_expression_astro_without_patient, y = NULL, fun = "cor", size = 2000, verbose=FALSE)
astro_cor <- as.data.frame(as.ffdf(astro_cor))

glio_cor <- bigcor(genes_expression_glio_without_patient, y = NULL, fun = "cor", size = 2000, verbose=FALSE)
glio_cor <- as.data.frame(as.ffdf(glio_cor))

## angular distance
ang_weight <- vector()

for (i in 1:dim(genes_expression_astro_without_patient)[2]){
ang_weight[i] <- acos(cosine(astro_cor[,i],glio_cor[,i]))/pi
}

## normalized weights (between 0 and 1)

astro_glio_weights <- ang_weight / max(ang_weight)
hist(ang_weight)
astro_glio_weights <- (1 - astro_glio_weights)^3
hist(astro_glio_weights, main="(1 - normal weight)^3")
# astro_glio_weights <- (astro_glio_weights)^3
# hist(astro_glio_weights, main="(normal weight)^3")
...
cv.fit_tweleele3 <- twiner_glmnet(prepareDataForCoxRegression(genes_expression_train),
                                  survival_object,0.3, astro_glio_weights)
##genes
