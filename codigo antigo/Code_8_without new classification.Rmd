...
compare_models <- function(models_coefficients, genes_expression_train, genes_expression_test,
  ...
  rownames(models_evaluation_df) <- c("GLMNET_Coefficients", "SIS_Coefficients", "ISIS_Coefficients")
...
nomes <- c("GLMNET", "SIS", "ISIS")

for (i in seq_along(list(GLMNET_Coefficients, SIS_Coefficients, ISIS_Coefficients))) {
  variable <- list(GLMNET_Coefficients, SIS_Coefficients, ISIS_Coefficients)[[i]]
  if (!is.null(variable$ensembl_gene_id)) {
    modelos_com_genes <- c(modelos_com_genes, list(variable$ensembl_gene_id))
  } else {
    nomes <- nomes[-i]  # Remove the corresponding model name if it has no coefficients
  }
}
