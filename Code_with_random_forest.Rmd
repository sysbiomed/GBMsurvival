...
# Prepare the data for the causal forest
survival_data <- survival_data[, -which(names(survival_data) %in% c("patient"))]

# Fit the Random Survival Forest model
set.seed(1012)
rsf_model <- rfsrc(Surv(days, vital_status) ~ ., data = survival_data)
...
# Extract coefficients from the fitted model
CausalForest_Coefficients <- data.frame(Coef_value = as.data.frame(coef(fit_rsf_model))[,1],                              ensembl_gene_id = rownames(as.data.frame(coef(fit_rsf_model))))
```
