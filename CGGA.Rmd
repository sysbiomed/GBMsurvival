...
if (!dir.exists("Results_CGGA")) {
  dir.create("Results_CGGA")
}
...
survival_data <- clinical_df[, c("CGGA_ID", "Histology", "OS", "Censor..alive.0..dead.1.")]
colnames(survival_data) <- c("patient", "project_id", "days", "vital_status")
...
for (i in seq_len(nrow(c_index_df))) {
  ...
  ci_cindex <- t.test(cindex_vals) #calculate_ci(cindex_vals)
  ci_ibs <- t.test(ibs_vals) #calculate_ci(ibs_vals)
  
  c_index_ci_lower[i] <- round(ci_cindex$conf.int[1], 3) # round(ci_cindex["lower"], 3)
  c_index_ci_upper[i] <- round(ci_cindex$conf.int[2], 3) # round(ci_cindex["upper"], 3)
  
  ibs_ci_lower[i] <- round(ci_ibs$conf.int[1], 3) # round(ci_ibs["lower"], 3)
  ibs_ci_upper[i] <- round(ci_ibs$conf.int[2], 3) # round(ci_ibs["upper"], 3)
}
...
write.csv(summary_table, file = "Results_CGGA/summary_table.csv", row.names = TRUE)
print(summary_table)

#Save a p-value as a CSV file
write.csv(p_value_df, file = "Results_CGGA/p_value_df.csv", row.names = TRUE)

#Save a p-value as a CSV file
write.csv(c_index_df, file = "Results_CGGA/c_index_df.csv", row.names = TRUE)
...
<!-- # Save to PDF -->
<!-- pdf("Results_CGGA/riskAUC.pdf", width = 6, height = 4) -->
...
<!-- # save the plot -->
<!-- pdf("Results_CGGA/upset_plot.pdf", width = 8, height = 3) # Create a PDF file to save the plot -->
<!-- replayPlot(recorded_plot) -->
...
<!-- #write.csv(Propo_hazards$table, file = "Results_CGGA/Propo_hazards", row.names = TRUE) -->

<!-- # Save a predictors genes and coefficients as a CSV file -->
<!-- # write.csv(gene_info_result, file = "Results_CGGA/predictors_genes_and_coefficients", row.names = FALSE) -->

<!-- # Write the captured output to a file -->
<!-- writeLines(survdiff_output, "Results_CGGA/survdiff_output.txt") -->
...
<!-- write.csv(riskAUC$Cindex, file = "Results_CGGA/concordance_IncidentCaseDynamicControlROC", row.names = FALSE) -->


<!-- # Save the table with the genes that are in more than 1 seed -->
<!-- write.csv(df_genes_in_multiple_seeds, file = "Results_CGGA/df_genes_in_multiple_seeds", row.names = FALSE) -->


<!-- # Save plots -->
<!-- ggsave(filename = "Results_CGGA/plot_high_low_risk.pdf", plot = plot$plot, width = 6, height = 4) -->
<!-- ggsave(filename = "Results_CGGA/Cumulative_ROC.pdf", plot = Cumulative_ROC, width = 6, height = 4) -->
<!-- ggsave(filename = "Results_CGGA/Incident_ROC.pdf", plot = Incident_ROC, width = 6, height = 4) -->
<!-- ggsave(filename = "Results_CGGA/distribution_of_risk_predictor.pdf", plot = plot_distribution, width = 6, height = 4) -->
...
<!-- #Save a summary table as a CSV file -->
<!-- write.csv(summary_table_all_equal, file = "Results_CGGA/summary_table_all_equal.csv", row.names = TRUE) -->
...
<!-- # Save to PDF -->
<!-- pdf("Results_CGGA/riskAUC_all_equal.pdf", width = 6, height = 4) # to save the plot below -->
<!-- replayPlot(recorded_plot) -->
<!-- dev.off() # to save the plot above -->
...
<!-- # Write the captured output to a file -->
<!-- writeLines(survdiff_output_all_equal, "Results_CGGA/survdiff_output_all_equal.txt") -->

<!-- # Save the concordance based on the Incident Case/Dynamic Control definition (calculated as a weighted average of the area under time-specific ROC) -->
<!-- write.csv(riskAUC_all_equal$Cindex, file = "Results_CGGA/concordance_IncidentCaseDynamicControlROC_all_equal", row.names = FALSE) -->

<!-- # Save plots -->
<!-- ggsave(filename = "Results_CGGA/plot_high_low_risk_all_equal.pdf", plot = plot_all_equal$plot, width = 6, height = 4) -->
<!-- ggsave(filename = "Results_CGGA/Cumulative_ROC_all_equal.pdf", plot = Cumulative_ROC_all_equal, width = 6, height = 4) -->
<!-- ggsave(filename = "Results_CGGA/Incident_ROC_all_equal.pdf", plot = Incident_ROC_all_equal, width = 6, height = 4) -->
<!-- ggsave(filename = "Results_CGGA/distribution_of_risk_predictor_all_equal.pdf", plot = plot_distribution_all_equal, width = 6, height = 4) -->
...
<!-- write.csv(comparison_test_train_data, file = "Results_CGGA/comparison_test_train_data.csv", row.names = FALSE) -->
...
<!-- # Write the captured output to a file -->
<!-- writeLines(survdiff_output_train_data, "Results_CGGA/survdiff_output_train_data.txt") -->

<!-- # Save plot -->
<!-- ggsave(filename = "Results_CGGA/plot_high_low_risk_train_data.pdf", plot = plot_train_data$plot, width = 6, height = 4) -->

<!-- ``` -->

<!-- # Kaplan-meier for some genes -->
...
<!-- # Define the gene to search for -->
<!-- gene_to_search <- "ENSG00000167779" -->

<!-- # Use grep to find the column that starts with the gene name -->
<!-- matching_column <- grep(paste0("^", gene_to_search, "\\."), colnames(genes_expression)) -->

<!-- # Check if a match is found -->
<!-- if (length(matching_column) > 0) { -->
<!--   print(paste("Column found:", colnames(genes_expression)[matching_column])) -->
<!-- } else { -->
<!--   print("Gene not found.") -->
<!-- } -->

<!-- # RPS27A = ENSG00000143947.14 -->
<!-- # LOXL1 = ENSG00000129038.16 -->
<!-- # IGFBP6 = ENSG00000167779.9 -->

<!-- gene_of_interest <- genes_expression$ENSG00000167779.9 # -->


<!-- # Categorize individuals  based on the median -->
<!-- risk <- ifelse(gene_of_interest > median(gene_of_interest), "High", "Low") -->
