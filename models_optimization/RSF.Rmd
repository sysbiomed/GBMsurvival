...
for (alpha in best_alpha_values) {
  ...
  for (i in seeds) {
    ...
    alpha_importance_list[[as.character(i)]] <- RSF_fit$importance_rsf
...
for (alpha in names(importance_rsf_list)) {
  for (seed in names(importance_rsf_list[[alpha]])) {
    imp_df <- importance_rsf_list[[alpha]][[seed]]
    
    if (!is.null(imp_df) && nrow(imp_df) > 0) {
      # Ensure 'importance' and optional 'gene_id' columns exist
      imp_df$Alpha <- alpha
      imp_df$Seed <- seed
      importance_combined <- rbind(importance_combined, imp_df)
    }
  }
}

# Optional: Remove zero importances (commonly added by RSF for non-selected features)
importance_combined <- importance_combined %>% filter(importance > 0)

# Convert Alpha to a factor for ordered plotting (optional)
importance_combined$Alpha <- factor(importance_combined$Alpha)

# Plot: Faceted density plots (one per alpha)
ggplot(importance_combined, aes(x = importance, fill = Alpha)) +
  geom_density(alpha = 0.6) +
  facet_wrap(~ Alpha, scales = "free_y") +
  theme_minimal() +
  labs(
    title = "Distribution of Feature Importances Across Seeds",
    x = "Importance Score",
    y = "Density"
  ) +
  theme(
